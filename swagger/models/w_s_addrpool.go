// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/swag"
)

// WSAddrpool w s addrpool
// swagger:model WSAddrpool
type WSAddrpool struct {

	// allow client classes
	AllowClientClasses []string `json:"allowClientClasses"`

	// container
	Container string `json:"container,omitempty"`

	// create date
	CreateDate string `json:"createDate,omitempty"`

	// deny client classes
	DenyClientClasses []string `json:"denyClientClasses"`

	// dhcp option set
	DhcpOptionSet string `json:"dhcpOptionSet,omitempty"`

	// dhcp policy set
	DhcpPolicySet string `json:"dhcpPolicySet,omitempty"`

	// end addr
	EndAddr string `json:"endAddr,omitempty"`

	// failover net service
	FailoverNetService string `json:"failoverNetService,omitempty"`

	// id
	ID string `json:"id,omitempty"`

	// last admin
	LastAdmin string `json:"lastAdmin,omitempty"`

	// last update date
	LastUpdateDate string `json:"lastUpdateDate,omitempty"`

	// name
	Name string `json:"name,omitempty"`

	// overlap interface Ip
	OverlapInterfaceIP bool `json:"overlapInterfaceIp,omitempty"`

	// prefix length
	PrefixLength string `json:"prefixLength,omitempty"`

	// primary net service
	PrimaryNetService string `json:"primaryNetService,omitempty"`

	// sharename
	Sharename string `json:"sharename,omitempty"`

	// start addr
	StartAddr string `json:"startAddr,omitempty"`

	// type
	Type string `json:"type,omitempty"`
}

// Validate validates this w s addrpool
func (m *WSAddrpool) Validate(formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *WSAddrpool) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *WSAddrpool) UnmarshalBinary(b []byte) error {
	var res WSAddrpool
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
