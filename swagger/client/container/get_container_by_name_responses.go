// Code generated by go-swagger; DO NOT EDIT.

package container

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	models "github.com/CMSgov/CMS-AWS-West-Network-Architecture/vpc-automation/swagger/models"
)

// GetContainerByNameReader is a Reader for the GetContainerByName structure.
type GetContainerByNameReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *GetContainerByNameReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewGetContainerByNameOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewGetContainerByNameDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewGetContainerByNameOK creates a GetContainerByNameOK with default headers values
func NewGetContainerByNameOK() *GetContainerByNameOK {
	return &GetContainerByNameOK{}
}

/*GetContainerByNameOK handles this case with default header values.

Container
*/
type GetContainerByNameOK struct {
	Payload *models.WSContainer
}

func (o *GetContainerByNameOK) Error() string {
	return fmt.Sprintf("[GET /Gets/getContainerByName][%d] getContainerByNameOK  %+v", 200, o.Payload)
}

func (o *GetContainerByNameOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.WSContainer)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewGetContainerByNameDefault creates a GetContainerByNameDefault with default headers values
func NewGetContainerByNameDefault(code int) *GetContainerByNameDefault {
	return &GetContainerByNameDefault{
		_statusCode: code,
	}
}

/*GetContainerByNameDefault handles this case with default header values.

Error response
*/
type GetContainerByNameDefault struct {
	_statusCode int

	Payload *models.Fault
}

// Code gets the status code for the get container by name default response
func (o *GetContainerByNameDefault) Code() int {
	return o._statusCode
}

func (o *GetContainerByNameDefault) Error() string {
	return fmt.Sprintf("[GET /Gets/getContainerByName][%d] getContainerByName default  %+v", o._statusCode, o.Payload)
}

func (o *GetContainerByNameDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.Fault)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
