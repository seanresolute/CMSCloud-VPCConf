...
type transitGatewayAttachmentTestCase struct {
	Name   string
	Region *string
...
func TestHandleTransitGatewayAttachments(t *testing.T) {
	vpcName := "test-vpc"
	vpcID := "vpc-123abc"
	accountID := "123789"
	var logger = &testLogger{}

	testCases := []*transitGatewayAttachmentTestCase{
		{
		...
		},
		{
			...
			PLResourceShares: []*PLResourceShare{
				{
					AccountID:       prefixListAccountIDCommercial,
					Region:          testRegion,
					PLIDs:           []string{"pl-lmn"},
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			for _, plrs := range tc.PLResourceShares {
				for _, id := range plrs.PLIDs {
					resources[plrs.ResourceShareID] = append(resources[plrs.ResourceShareID], testmocks.MockResource{
						ID:   prefixListARN(testRegion, prefixListAccountIDCommercial, id),
			...
			plRAM := &testmocks.MockRAM{
				AccountID:               prefixListAccountIDCommercial,
				Region:                  region,
				ResourceSharePrincipals: tc.ResourceSharePrincipals[prefixListAccountIDCommercial],
				ResourceShareResources:  resources,
				ResourceShareStatuses:   statuses,
				TestRegion:              testRegion,
			}
			shareRAMs[prefixListAccountIDCommercial] = plRAM
			getPrefixListRAM := func(region database.Region) (ramiface.RAMAPI, error) {
				return shareRAMs[prefixListAccountIDCommercial], nil
			}

			var expectedErrorString string
			if tc.UnsharedPLID != "" {
				expectedErrorString = fmt.Sprintf("Error ensuring configured Prefix Lists are shared via RAM: The configured Prefix List %s is not yet shared. You must share the Prefix List from account %s in the AWS Console first.", tc.UnsharedPLID, prefixListAccountIDCommercial)
			}
			...
			for accountID, ram := range shareRAMs {
				...
				for shareID, principals := range tc.SharesUpdated[accountID] {
					expectedSharesUpdated[resourceShareARN(testRegion, accountID, shareID)] = principals
				}
			...
			for accountID, acctShares := range tc.SharesUpdated {
				for shareID := range acctShares {
					expectedARNs = append(expectedARNs, resourceShareARN(testRegion, accountID, shareID))
				}
			}
